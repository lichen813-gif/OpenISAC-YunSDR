...
private:
    ...
    bool _try_inject_ertm_payload() {
        ...
        const size_t packet_symbols = LdpcPacketFraming::packet_symbols(
            payload_blocks, modulation_bits_per_symbol(_cfg.downlink.modulation));
        if (packet_symbols > _data_resource_layout.payload_re_count) {
            LOG_G_WARN_M(Ertm) << "[eRTM] TO payload does not fit in one downlink frame; dropping measurement"
                         << ": payload_bytes=" << payload.size()
                         << ", physical_syms=" << packet_symbols
    ...
    bool _encode_and_enqueue_payload(
        ...
        const size_t payload_bits_per_symbol =
            modulation_bits_per_symbol(_cfg.downlink.modulation);
        const size_t packet_symbols =
            LdpcPacketFraming::packet_symbols(payload_blocks, payload_bits_per_symbol);
        if (packet_symbols > _data_resource_layout.payload_re_count) {
        ...
        const uint8_t header_flags = static_cast<uint8_t>(
            ...
            LdpcPacketFraming::modulation_flag_from_bits(payload_bits_per_symbol));
        ...
        if (payload_blocks > 0) {
            ...
            LDPCCodec::pack_bits_modulation(
                _ldpc_encoded_bits_scratch,
                payload_bits_per_symbol,
                _ldpc_qpsk_scratch);
            for (int& symbol : _ldpc_qpsk_scratch) {
                symbol = LdpcPacketFraming::tag_payload_symbol(symbol);
            }
        }
        prof_step_end = ProfileClock::now();
        ...
        if (should_track_arq) {
            ...
            const int64_t tx_due_ms =
                now + _estimate_arq_queue_delay_ms(queued_packets_ahead, packet_symbols);
     ...
     * 4. Maps interleaved bits to configured square-QAM payload symbols.
            ...
            if (_arq_enabled) {
                ...
                for (uint16_t seq : retransmit_seqs) {
                    ...
                    if (!entry.encoded_qpsk.empty()) {
                        // Cheap retransmit: push pre-encoded control/QAM symbols directly
                        ...
                        const size_t packet_symbols =
                            LdpcPacketFraming::packet_symbols(
                                payload_blocks,
                                modulation_bits_per_symbol(_cfg.downlink.modulation));
                        const int64_t estimated_tx_ms =
                            now + _estimate_arq_queue_delay_ms(encoded_size, packet_symbols);
                        _arq_dl_retransmit_enqueued.fetch_add(1, std::memory_order_relaxed);
                        _dl_arq_tx.mark_transmitted(seq, estimated_tx_ms);
                    }
                }
     ...
     * 2. Maps QPSK control and configured-QAM payload labels to constellation points.
    ...
    void _modulation_proc() {
        ...
        while (_running.load()) {
                ...
                if (pkt_size > room) {
                    ...
                    if (pkt_size > max_data_syms_per_frame) {
                        ...
                        if (oversize_head_warn_count <= 20 || (oversize_head_warn_count % 100) == 0) {
                            LOG_RT_WARN_M(ModLdpc) << "[LDPC] Queue head packet exceeds per-frame capacity, not fetched: physical_syms="
                                          << pkt_size << ", max_physical_syms_per_frame=" << max_data_syms_per_frame
            ...
            for (size_t payload_idx = 0; payload_idx < data_pool.size(); ++payload_idx) {
                const int encoded_symbol = data_pool_ptr[payload_idx];
                if (LdpcPacketFraming::is_payload_symbol(encoded_symbol)) {
                    mod_ptr[payload_idx] = SquareQAM::modulate(
                        static_cast<unsigned>(
                            LdpcPacketFraming::payload_symbol_value(encoded_symbol)),
                        modulation_bits_per_symbol(_cfg.downlink.modulation));
                } else {
                    mod_ptr[payload_idx] = _qpsk_symbol_from_int(encoded_symbol);
                }
            }
