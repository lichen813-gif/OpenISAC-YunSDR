...
void LDPCCodec::pack_bits_modulation(const AlignedIntVector& bits,
                                     size_t bits_per_symbol,
                                     AlignedIntVector& symbols) {
    if (bits_per_symbol != 2 && bits_per_symbol != 4 &&
        bits_per_symbol != 6 && bits_per_symbol != 8) {
        throw std::invalid_argument("pack_bits_modulation supports Qm=2,4,6,8 only");
    }
    const size_t bit_count = bits.size();
    const size_t symbol_count =
        (bit_count + bits_per_symbol - 1) / bits_per_symbol;
    symbols.resize(symbol_count);
    for (size_t symbol_idx = 0; symbol_idx < symbol_count; ++symbol_idx) {
        int label = 0;
        for (size_t bit = 0; bit < bits_per_symbol; ++bit) {
            const size_t source_idx = symbol_idx * bits_per_symbol + bit;
            label <<= 1;
            if (source_idx < bit_count) {
                label |= bits[source_idx] & 1;
            }
        }
        symbols[symbol_idx] = label;
    }
}
