        ...
        : cfg_(cfg),
          ...
          _llr_pool(32, [&cfg]() {
              ...
              return AlignedFloatVector(
                  layout.payload_re_count *
                  modulation_bits_per_symbol(cfg.downlink.modulation));
          }),
          _llr_pool_i16(32, [&cfg]() {
              const DataResourceGridLayout layout = build_data_resource_grid_layout(cfg);
              return LDPCCodec::AlignedShortVector(
                  layout.payload_re_count *
                  modulation_bits_per_symbol(cfg.downlink.modulation));
          }),
...
private:
    ...
    std::vector<int> _payload_symbol_indices_flat;
    ...
    void _build_compact_payload_indices() {
        ...
        _payload_symbol_indices_flat.clear();
        _payload_subcarrier_indices_flat.reserve(_data_resource_layout.payload_re_count);
        _payload_symbol_indices_flat.reserve(_data_resource_layout.payload_re_count);
        ...
        for (size_t sym_idx = 0; sym_idx < _data_resource_layout.data_symbol_count; ++sym_idx) {
            ...
            for (size_t di = 0; di < _data_resource_layout.num_non_pilot_subcarriers; ++di) {
                ...
                _payload_symbol_indices_flat.push_back(static_cast<int>(sym_idx));
            }
            if (_payload_subcarrier_indices_flat.size() != _data_resource_layout.payload_offsets[sym_idx + 1] ||
                _payload_symbol_indices_flat.size() != _data_resource_layout.payload_offsets[sym_idx + 1]) {
    ...
    std::complex<float> _nearest_mixed_payload_reference(
        std::complex<float> sample) const {
        const std::complex<float> qpsk_ref(
            std::copysign(QPSKModulator::SQRT_2_INV, sample.real()),
            std::copysign(QPSKModulator::SQRT_2_INV, sample.imag()));
        const size_t bits_per_symbol =
            modulation_bits_per_symbol(cfg_.downlink.modulation);
        if (bits_per_symbol == 2) {
            return qpsk_ref;
        }
        const std::complex<float> qam_ref =
            SquareQAM::remodulate(sample, bits_per_symbol);
        return std::norm(sample - qam_ref) < std::norm(sample - qpsk_ref)
            ? qam_ref
            : qpsk_ref;
    }

    void _remodulate_mixed_symbol(const AlignedVector& equalized_symbol,
                                  AlignedVector& output) const {
        output.resize(equalized_symbol.size());
        for (size_t k = 0; k < equalized_symbol.size(); ++k) {
            output[k] = _nearest_mixed_payload_reference(equalized_symbol[k]);
        }
        for (const size_t pilot : cfg_.ofdm.pilot_positions) {
            if (pilot < output.size()) {
                output[pilot] = zc_freq_[pilot];
            }
        }
    }
        ...
        for (size_t sym_idx = 0; sym_idx < symbols.size(); ++sym_idx) {
            ...
            for (size_t idx = payload_begin; idx < payload_end; ++idx) {
                ...
                const std::complex<float> ref =
                    _nearest_mixed_payload_reference(sym_ptr[k]);
                const float err_re = sym_ptr[k].real() - ref.real();
                const float err_im = sym_ptr[k].imag() - ref.imag();
...
//        LOG_G_INFO_M(Root) <<  discard_samples_<< std::endl;
    ...
    template<typename LlrVector>
    bool _demap_downlink_packet_llrs(const std::vector<AlignedVector>& symbols,
                                     float llr_scale_snapshot,
                                     LlrVector& output) const {
        using LlrValue = typename LlrVector::value_type;
        constexpr bool kFixedPoint = std::is_same<LlrValue, int16_t>::value;
        const size_t configured_bits_per_symbol =
            modulation_bits_per_symbol(cfg_.downlink.modulation);
        const float fixed_point_scale = kFixedPoint
            ? static_cast<float>(cfg_.ldpc.fixed_point_scale)
            : 1.0f;
        const float control_scale = std::min(
            llr_scale_snapshot * static_cast<float>(M_SQRT1_2), 500.0f);
        const float noise_variance =
            4.0f / std::max(llr_scale_snapshot, 1.0e-6f);

        output.clear();
        output.reserve(
            _data_resource_layout.payload_re_count * configured_bits_per_symbol);

        auto equalized_at = [&](size_t physical_index) -> std::complex<float> {
            const size_t symbol_index = static_cast<size_t>(
                _payload_symbol_indices_flat[physical_index]);
            const size_t subcarrier_index = static_cast<size_t>(
                _payload_subcarrier_indices_flat[physical_index]);
            return symbols[symbol_index][subcarrier_index];
        };
        auto append_llr = [&](float value) {
            const float clamped = std::max(-500.0f, std::min(500.0f, value));
            if constexpr (kFixedPoint) {
                output.push_back(sat16_llr(clamped * fixed_point_scale));
            } else {
        ...
        std::array<float, LdpcPacketFraming::kControlBits> control_llrs{};
        std::array<float, 8> payload_llrs{};
        while (physical_offset + LdpcPacketFraming::kControlSymbols <=
               _data_resource_layout.payload_re_count) {
            for (size_t symbol_idx = 0;
                 symbol_idx < LdpcPacketFraming::kControlSymbols;
                 ++symbol_idx) {
                const std::complex<float> sample =
                    equalized_at(physical_offset + symbol_idx);
                control_llrs[symbol_idx * 2] = sample.real() * control_scale;
                control_llrs[symbol_idx * 2 + 1] = sample.imag() * control_scale;
            }

            if (!LdpcPacketFraming::detect_marker_llrs(control_llrs.data())) {
                break;
            }
            LdpcMiniHeader mini_header;
            if (!LdpcPacketFraming::decode_mini_header_llrs(
                    control_llrs.data() + LdpcPacketFraming::kMarkerBits,
                    mini_header)) {
                break;
            }

            const size_t packet_bits_per_symbol =
                LdpcPacketFraming::modulation_bits_from_flags(mini_header.flags);
            if (packet_bits_per_symbol != configured_bits_per_symbol) {
                LOG_RT_WARN_HZ_M(Demod, 2)
                    << "[Demod] Dropping frame because mini-header modulation Qm="
                    << packet_bits_per_symbol << " differs from configured Qm="
                    << configured_bits_per_symbol << '.';
                break;
            }
            const size_t payload_blocks =
                LdpcPacketFraming::payload_blocks_for_len(mini_header.payload_len);
            const size_t payload_physical_symbols =
                payload_blocks *
                (LdpcPacketFraming::kLdpcCodeBitsPerBlock / packet_bits_per_symbol);
            const size_t next_physical_offset =
                physical_offset + LdpcPacketFraming::kControlSymbols +
                payload_physical_symbols;
            if (next_physical_offset > _data_resource_layout.payload_re_count) {
                break;
            }

            for (float value : control_llrs) {
                append_llr(value);
            }
            for (size_t symbol_idx = 0;
                 symbol_idx < payload_physical_symbols;
                 ++symbol_idx) {
                SquareQAM::compute_llrs(
                    equalized_at(
                        physical_offset + LdpcPacketFraming::kControlSymbols +
                        symbol_idx),
                    noise_variance,
                    packet_bits_per_symbol,
                    payload_llrs.data());
                for (size_t bit = 0; bit < packet_bits_per_symbol; ++bit) {
                    append_llr(payload_llrs[bit]);
                }
            }
    ...
    void _run_cpu_demod_task(CpuDemodWorkerContext& ctx, CpuDemodTask&& task, CpuDemodResult& result) {
        ...
        if (_data_resource_layout.payload_re_count > 0) {
            if (_ldpc_fixed_point) {
                ...
                if (_demap_downlink_packet_llrs(
                        ctx.symbols, task.llr_scale_snapshot, result.llr_i16)) {
                ...
                if (_demap_downlink_packet_llrs(
                        ctx.symbols, task.llr_scale_snapshot, result.llr)) {
    ...
    void _decode_llr_frame(const LlrVec& llr,
        ...
        size_t bit_offset = 0;
        while (bit_offset + LdpcPacketFraming::kControlBits <= llr.size()) {
            const size_t control_llr_offset = bit_offset;
            ...
            const size_t next_bit_offset =
                payload_llr_offset + required_llr;
