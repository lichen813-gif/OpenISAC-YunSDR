...
def ensure_distinct_control_ports(mod_cfg: dict, demod_cfg: dict) -> None:
    ...
    demod_port = int(demod_cfg.get("control_port", 10001))
...
def estimate_single_frame_payload_limit(cfg: dict) -> int:
    ...
    ofdm_cfg = cfg.get("ofdm_frame", cfg)
    downlink_cfg = cfg.get("downlink", cfg)
    fft_size = int(ofdm_cfg.get("fft_size", 1024))
    num_symbols = int(ofdm_cfg.get("num_symbols", 100))
    pilot_positions = ofdm_cfg.get("pilot_positions", [])
    modulation = str(downlink_cfg.get("modulation", "qpsk")).lower().replace("-", "")
    bits_per_symbol = {
        "qpsk": 2,
        "4qam": 2,
        "16qam": 4,
    ...
    physical_res = max(0, num_symbols - 1) * data_subcarriers
    control_symbols = 128  # marker (64) + BCH mini-header (64)
    payload_symbols_per_block = ldpc_n_bits // bits_per_symbol
    payload_blocks = max(0, physical_res - control_symbols) // payload_symbols_per_block
