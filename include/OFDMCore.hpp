...
public:
    ...
    // Mini-header flag bits (4-bit field). Low bits classify control/internal
    // frames; high bits signal payload Qm. Unknown flag bits are rejected.
    static constexpr uint8_t kFlagArqFeedback = 0x01;
    static constexpr uint8_t kFlagErtmTiming = 0x02;
    static constexpr uint8_t kFlagModulationMask = 0x0C;
    static constexpr uint8_t kFlagModulationQpsk = 0x00;
    static constexpr uint8_t kFlagModulation16Qam = 0x04;
    static constexpr uint8_t kFlagModulation64Qam = 0x08;
    static constexpr uint8_t kFlagModulation256Qam = 0x0C;
    static constexpr uint8_t kKnownFlagsMask = 0x0F;
    // Encoded packet buffers contain both robust QPSK control symbols and QAM
    // payload symbols.  This tag is internal only and never appears on air.
    static constexpr int kPayloadSymbolTag = 0x100;
    static constexpr int kPayloadSymbolMask = 0x0FF;
    ...
    static uint8_t modulation_flag_from_bits(size_t bits_per_symbol) {
        switch (bits_per_symbol) {
        case 2: return kFlagModulationQpsk;
        case 4: return kFlagModulation16Qam;
        case 6: return kFlagModulation64Qam;
        case 8: return kFlagModulation256Qam;
        default:
            throw std::runtime_error("Unsupported payload modulation order in LDPC framing.");
        }
    }

    static size_t modulation_bits_from_flags(uint8_t flags) {
        switch (flags & kFlagModulationMask) {
        case kFlagModulationQpsk: return 2;
        case kFlagModulation16Qam: return 4;
        case kFlagModulation64Qam: return 6;
        case kFlagModulation256Qam: return 8;
        default: return 2;
        }
    }

    static int tag_payload_symbol(int symbol) {
        return kPayloadSymbolTag | (symbol & kPayloadSymbolMask);
    }

    static bool is_payload_symbol(int symbol) {
        return (symbol & kPayloadSymbolTag) != 0;
    }

    static int payload_symbol_value(int symbol) {
        return symbol & kPayloadSymbolMask;
    }
    ...
    static size_t packet_symbols(size_t payload_blocks, size_t bits_per_symbol) {
        if (!SquareQAM::supported_bits_per_symbol(bits_per_symbol) ||
            (kLdpcCodeBitsPerBlock % bits_per_symbol) != 0) {
            throw std::runtime_error("LDPC block size is incompatible with payload modulation.");
        }
        return kControlSymbols +
               payload_blocks * (kLdpcCodeBitsPerBlock / bits_per_symbol);
    }
