...
inline constexpr const char* kModulationQpsk = "qpsk";
inline constexpr const char* kModulation16Qam = "16qam";
inline constexpr const char* kModulation64Qam = "64qam";
inline constexpr const char* kModulation256Qam = "256qam";

inline std::string normalize_modulation_string(std::string modulation) {
    std::transform(modulation.begin(), modulation.end(), modulation.begin(), [](unsigned char ch) {
        return static_cast<char>(std::tolower(ch));
    });
    modulation.erase(
        std::remove_if(modulation.begin(), modulation.end(), [](unsigned char ch) {
            return ch == '-' || ch == '_' || std::isspace(ch);
        }),
        modulation.end());
    if (modulation == "qpsk" || modulation == "4qam" || modulation == "qam4") {
        return kModulationQpsk;
    }
    if (modulation == "16qam" || modulation == "qam16") {
        return kModulation16Qam;
    }
    if (modulation == "64qam" || modulation == "qam64") {
        return kModulation64Qam;
    }
    if (modulation == "256qam" || modulation == "qam256") {
        return kModulation256Qam;
    }
    throw std::runtime_error(
        "Invalid modulation='" + modulation +
        "'. Supported values: qpsk, 16qam, 64qam, 256qam.");
}

inline size_t modulation_bits_per_symbol(const std::string& modulation) {
    const std::string normalized = normalize_modulation_string(modulation);
    if (normalized == kModulationQpsk) return 2;
    if (normalized == kModulation16Qam) return 4;
    if (normalized == kModulation64Qam) return 6;
    return 8;
}
...
inline void normalize_bs_sensing_channels(Config& cfg) {
    cfg.downlink.modulation = normalize_modulation_string(cfg.downlink.modulation);
