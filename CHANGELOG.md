...
新增源码级 `libyunsdr-isac/` 子项目，在不恢复 UHD/USRP 实现的前提下接入
YunSDR Y240，并保留主工程的软件仿真后端和厂商无关无线设备边界。

### Changes

- 加入已验证的 Y240 `pcies:0.0` 后端、时间戳收发流、设备探测和射频回环工具。
- 复用正式 OpenISAC PHY 编解码，支持 SISO、2x2 双层空间复用和 2x2 Alamouti STBC。
- 加入 VLC/UDP 硬件视频桥和实时 Python 监视器源码；视频文件不进入仓库。
- 发布完整 C++ 源码、头文件、YAML 配置、VS2019 构建/运行脚本以及中英文接入说明。
- 厂商 SDK、头文件、DLL、LIB、PDB、驱动、固件、构建产物、IQ 数据和硬件日志均由
  `.gitignore` 排除，避免分发受限或与本机绑定的文件。

### Validation

- 全新软件后端构建通过，CTest 3/3 通过：基础接口、收发器状态机、正式 PHY 编解码。
- 主 PHY 全新构建通过，CTest 9/9 通过；Python 算法、脚本和感知前端测试 119/119 通过。
- 本次发布整理未重新执行实时射频测试；已验证的硬件过程和结果保留在 Y240 文档中。

## 2026-08-24 - v1.1.0 仿真后端与发布文档整理

### Summary

当前公开版本收敛为仅支持 ChannelSimulator 的软件仿真平台，完整移除 UHD/USRP 实现、依赖、硬件配置和工具残留，并重新整理中英文搭建、使用、物理层参数与 Python 测试说明。

### Changes

- 删除 UHD 后端源码、CMake 依赖、硬件设备/PPS 配置及 X310/B210 示例；非 `sim` 后端在配置加载阶段会被明确拒绝。
- 保留厂商无关的 `RadioBackend` 边界作为后续 `libyunsdr` 接入点；在真实 SDK 后端完成验证前不声明具体接口兼容性。
- Docker、性能测试模板、CPU 隔离工具和 Web 配置编辑器统一改为 ChannelSimulator 配置与逻辑流通道。
- 中英文首页改为编译、运行、运行时 PHY 参数、正式帧兼容参数和配置约束说明，并移除首页 Logo。
- 合并 Rank-4 时域/正式帧链路、NR 风格双符号 DM-RS、信道估计、TDL 信道、采样偏差处理以及 4×4 诊断与基准工具。
- 确认并发布完整 Python PHY 算法模块、实验脚本、配置和测试说明；根依赖补充 `pytest>=7.0`，清除文档中的旧本地绝对路径。
- 补齐 C++ 正式帧回归所需的非视频黄金向量；本版本不包含视频文件。

### Validation

- Python PHY：114 项测试通过；包含脚本与感知前端测试时共 119 项通过。
- Windows C++ 正式 PHY：干净构建成功，CTest 4/4 通过（含 Rank-1/2/4 兼容自检）。
- 18 份 YAML 解析和 8 份 Web 配置渲染通过；实现与工具目录的 UHD/USRP 残留扫描为零。
